...
Self-host also exposes the freshness window as `EXECUTOR_TOOLS_SYNC_TTL_MS`. Leave it unset for the 15-minute default, or set `0` (equivalently `off`, `null` or `false`) to disable time-based re-sync and leave stale-marking and config revision as the only refresh triggers. A malformed or negative value is refused at boot rather than silently falling back to the default.
